...
function serializeNode(node: HastNode): string {
  ...
  if (node.type === 'raw') return escapeHtml(node.value ?? '')
  if (node.type !== 'element') return (node.children ?? []).map(serializeNode).join('')
  const tag = node.tagName ?? 'div'
  const attrs = Object.entries(node.properties ?? []).flatMap(([name, value]) => {
    if (value === null || value === undefined || value === false) return []
    const attr = attributeName(name)
    if (value === true) return [attr]
    const normalized = Array.isArray(value) ? value.join(' ') : String(value)
    return [`${attr}="${escapeHtml(normalized)}"`]
  }).join(' ')
  const open = `<${tag}${attrs ? ` ${attrs}` : ''}>`
  if (VOID_HTML_TAGS.has(tag)) return open
  return `${open}${(node.children ?? []).map(serializeNode).join('')}</${tag}>`
}

export function renderMarkdownBody(markdown: string): string {
  const processor = remark().use(remarkGfm).use(remarkMath).use(remarkStandaloneDisplayMath).use(remarkRehype).use(rehypeKatex)
  const tree = processor.runSync(processor.parse(normalizeLatexBlockDelimiters(markdown))) as unknown as HastNode
  const body = serializeNode(tree)
  return body.replace(/<code class="language-mermaid">([\s\S]*?)<\/code>/g, '<pre class="mermaid">$1</pre>')
}
