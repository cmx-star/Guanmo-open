...
export function registerBuiltinTools() {
  registerTool({
    ...
    description: '在本地 RAG 数据库中检索已索引文档内容。可查询未打开、未添加到聊天框上下文的文档；工具返回文件路径、行号、标题路径和正文片段，可直接用于基于知识库的问答、归纳和局部总结。只有需要精确读取整份原文或改写文件时，才要求用户添加目标文件上下文。首次查询会在后台初始化索引；初始化失败时自动改用关键词检索，不应要求用户稍后重试。',
    parameters: [
